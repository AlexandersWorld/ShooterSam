...
void AShooterSamCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		EnhancedInputComponent->BindAction(ChangeWeaponAction, ETriggerEvent::Started, this, &AShooterSamCharacter::ChangeWeapon);
	}
	else
	{
...
void AShooterSamCharacter::ChangeWeapon()
{
	if (GunClasses.Num() <= 1) return;

	CurrentWeaponIndex = (CurrentWeaponIndex + 1) % GunClasses.Num();
	SpawnWeapon(CurrentWeaponIndex);
}

void AShooterSamCharacter::SpawnWeapon(int32 WeaponIndex)
{
	if (!GunClasses.IsValidIndex(WeaponIndex)) return;
	...
	CurrentGun = GetWorld()->SpawnActor<AGun>(GunClasses[WeaponIndex]);

	EquipWeapon(CurrentGun);
}

void AShooterSamCharacter::EquipWeapon(AGun* Gun)
{
	if (Gun)
	{
		Gun->SetOwner(this);
